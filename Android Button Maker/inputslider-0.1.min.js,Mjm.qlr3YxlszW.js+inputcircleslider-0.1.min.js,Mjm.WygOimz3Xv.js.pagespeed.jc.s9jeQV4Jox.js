	...
	a.fn.iCircleSlider = function(p) {
		...
		return this.each(function() {
			...
			var f = a(c.id).val(),
				...
				l = h,
				b = 0;
			a(c.id).on("click", function() {
				if (!1 == e) {
					f = a(c.id).val();
					f = c.degbound * Math.round(f / c.degbound);
					m = a(c.id).offset();
					n = a(c.id).outerHeight();
					var d = a("<div class='circlecontainer' ></div>").css({
						top: m.top + n,
						left: m.left,
						position: "absolute"
					}).append("<div id='dial_0'>0</div><div id='dial_90'>90</div><div id='dial_180'>180</div><div id='dial_270'>270</div><div id='r_pt'></div><div id='dial'><div id='dial_l' >&nbsp;</div></div>");
					a("body").append(d);
					d.attr("unselectable", "on").css("UserSelect", "none").css("MozUserSelect", "none");
					calct = 180 - f;
					a("#dial").css({
					...
					d.mousedown(function(d) {
						...
						k = e.left - g;
						l = e.top - h;
						b = Math.round(Math.atan2(l, k) * (180 / Math.PI));
						a("#dial").css({
			...
			a(document).mousemove(function(d) {
				!0 == e && (g = d.pageX, h = d.pageY, j && (d = a("#r_pt").offset(), k = d.left - g, l = d.top - h, b = Math.round(Math.atan2(l, k) * (180 / Math.PI)), a("#dial").css({
					transform: "rotate(" + b + "deg)",
					"-ms-transform": "rotate(" + b + "deg)",
					"-webkit-transform": "rotate(" + b + "deg)",
					"-o-transform": "rotate(" + b + "deg)",
